...
"""
    ...
    def _select_best_answer(self, video_result: Dict, knowledge_result: Dict, question: str) -> Dict:
        ...
        try:
            ...
            # Pure relevance-based selection - no source bias
            # Always select the source with the highest relevance score
            if video_result.get('success') and knowledge_result.get('success'):
                # Both sources available - compare relevance scores
                if video_score > knowledge_score:
                    print(f"🎬 Video more relevant ({video_score:.3f} > {knowledge_score:.3f}) - using video answer with timestamp")
                    return self._generate_guided_answer(video_result, question)
                elif knowledge_score > video_score:
                    print(f"📚 Knowledge more relevant ({knowledge_score:.3f} > {video_score:.3f}) - using knowledge answer only")
                    return {
                        'success': True,
                    ...
                    # Equal relevance scores - combine both sources
                    print(f"🔄 Equal relevance scores ({video_score:.3f} = {knowledge_score:.3f}) - combining both sources")
                ...
                print(f"🎬 Only video available ({video_score:.3f}) - using video answer with timestamp")
                return self._generate_guided_answer(video_result, question)
            elif knowledge_result.get('success'):
                # Only knowledge available
                print(f"📚 Only knowledge available ({knowledge_score:.3f}) - using knowledge answer only")
                return {
                    'success': True,
                    'answer': knowledge_result['answer'],  # Already formatted by _format_knowledge_answer
                    'start': 0,
            ...
            # No sources available - return error
