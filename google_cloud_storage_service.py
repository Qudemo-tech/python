...
class GoogleCloudStorageService:
    ...
    def search_transcript_for_question(self, company_name: str, qudemo_id: str, 
        ...
        try:
            ...
            # Prepare question for context-aware matching
            question_lower = question.lower()
            question_keywords = [word for word in question_lower.split() if len(word) > 2]  # Filter short words
            
            # Define content patterns for better context matching
            content_patterns = {
                'disqualified_leads': [
                    'disqualified lead', 'disqualified prospect', 'not qualified', 'unqualified',
                    'disqualified agent', 'disqualified call', 'disqualified status',
                    'lead is not qualified', 'when a lead is not qualified'
                ],
                'qualified_leads': [
                    'qualified lead', 'qualified prospect', 'qualified agent',
                    'qualified call', 'qualified status', 'lead is qualified',
                    'when a lead is qualified', 'qualified deal'
                ],
                'general_lead_management': [
                    'lead management', 'lead process', 'lead workflow',
                    'prospect management', 'lead handling', 'lead follow-up'
                ]
            }
            
            # Find relevant chunks with context-aware scoring
            relevant_chunks = []
            ...
            for chunk in chunks:
                ...
                # Calculate base keyword relevance
                ...
                # SPECIAL BYPASS: Always include introduction content that explains what the video is about
                # BUT only for very general questions, not for specific "qualified" or "disqualified" questions
                is_very_general_question = any(term in question_lower for term in ['how to build', 'build agents', 'build browser agents']) and not any(term in question_lower for term in ['qualified', 'disqualified'])
                is_specific_qualified_question = any(term in question_lower for term in ['qualified', 'disqualified'])
                is_specific_test_question = any(term in question_lower for term in ['how to test', 'test', 'testing'])
                
                if ('building two browser agents' in chunk_text or 'one for qualified leads and one for disqualified leads' in chunk_text) and is_very_general_question and not is_specific_qualified_question:
                    base_relevance = 1.0  # Force maximum base relevance for introduction content
                    print(f"🎯 FOUND INTRODUCTION CHUNK: {chunk_metadata.get('segment_start', '00:00')} - {chunk_text[:100]}...")
                
                # Calculate context relevance based on content patterns
                context_relevance = 0
                
                # Check for disqualified leads content patterns
                if any(term in question_lower for term in ['disqualified', 'unqualified', 'not qualified']):
                    # HEAVILY PENALIZE qualified content when asking about disqualified leads
                    if 'qualified' in chunk_text and 'disqualified' not in chunk_text:
                        context_relevance -= 2.0  # Heavy penalty for qualified-only content
                    
                    for pattern in content_patterns['disqualified_leads']:
                        if pattern in chunk_text:
                            context_relevance += 0.4
                    
                    # Special bonus for specific "disqualified lead" phrase
                    if 'disqualified lead' in chunk_text:
                        context_relevance += 0.6  # High bonus for exact phrase
